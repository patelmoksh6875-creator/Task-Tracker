export const DataEngine = {
    ...
    // Persistent record of which calendar days had at least one completed
    // task — kept forever (never cleared by the monthly rollover) so a
    // streak counter can span month boundaries correctly.
    getActiveDays() {
        const local = localStorage.getItem('cs_active_days');
        return local ? JSON.parse(local) : [];
    },
    saveActiveDays(days) { localStorage.setItem('cs_active_days', JSON.stringify(days)); },
    recordActiveDay(dayKey) {
        const days = this.getActiveDays();
        if (!days.includes(dayKey)) {
            days.push(dayKey);
            this.saveActiveDays(days);
        }
    },
    ...
    addCompletedLog(task, goalTitle) {
        ...
        this.recordActiveDay(this.todayKey());
    },

    // Timeline: long-range events (up to a year or two out) with a lead-time
    // window telling you when to start preparing, not just when the event
    // itself lands.
    getTimelineEvents() {
        const local = localStorage.getItem('cs_timeline_events');
        return local ? JSON.parse(local) : [];
    },
    saveTimelineEvents(events) { localStorage.setItem('cs_timeline_events', JSON.stringify(events)); }
};
...
export const UIManager = {
    ...
    setupDashboardPage() {
        ...
        this.renderTimelineRemindersCard();
    },
    ...
    // --- TIMELINE MODULE ---
    // Long-range events (up to a year or two out) with a lead-time window:
    // 'past' = date has gone, 'today' = it's happening today, 'active' =
    // today falls inside the reminder window (time to start prepping),
    // 'upcoming' = further out, reminder window hasn't opened yet.
    getTimelineStatus(event) {
        ...
        if (daysUntil < 0) return { state: 'past', daysUntil };
        if (daysUntil === 0) return { state: 'today', daysUntil };
        if (daysUntil <= event.leadDays) return { state: 'active', daysUntil };
    ...
    setupTimelinePage() {
        ...
        form.addEventListener('submit', (e) => {
            ...
            const leadDays = parseInt(document.getElementById('timeline-lead').value, 10);
            const note = document.getElementById('timeline-note').value.trim();
            if (!title || !eventDate || isNaN(leadDays) || leadDays < 0) return;

            const events = DataEngine.getTimelineEvents();
            events.push({ id: generateId(), title, eventDate, leadDays, note });
            DataEngine.saveTimelineEvents(events);

            form.reset();
            document.getElementById('timeline-lead').value = 7;
    ...
    renderTimelineList() {
        ...
        if (events.length === 0) {
            container.innerHTML = `<p class="reflect-card-detail" style="padding: 8px 4px;">Nothing on the timeline yet — add your first event above.</p>`;
            return;
        }

        container.innerHTML = events.map(event => {
            const { state, daysUntil } = this.getTimelineStatus(event);
            const dateLabel = new Date(event.eventDate + 'T00:00:00').toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });

            let statusLabel;
            if (state === 'past') statusLabel = 'Passed';
            else if (state === 'today') statusLabel = 'Today';
            else if (state === 'active') statusLabel = `${daysUntil} day${daysUntil === 1 ? '' : 's'} away — start preparing`;
            else statusLabel = `${daysUntil} days away`;

            return `
                <div class="timeline-entry timeline-entry-${state}" data-id="${event.id}">
                    <div class="timeline-entry-main">
                        <div class="timeline-entry-title">${event.title}</div>
                        <div class="timeline-entry-date">${dateLabel} · ${statusLabel}</div>
                        ${event.note ? `<div class="timeline-entry-note">${event.note}</div>` : ''}
                    </div>
                    <button type="button" class="timeline-entry-delete" aria-label="Remove event" title="Remove">✕</button>
                </div>
            `;
        }).join('');

        container.querySelectorAll('.timeline-entry-delete').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const id = e.target.closest('.timeline-entry').dataset.id;
                const events = DataEngine.getTimelineEvents().filter(ev => ev.id !== id);
                DataEngine.saveTimelineEvents(events);
                this.renderTimelineList();
            });
        });
    ...
    // Dashboard-facing summary: only surfaces events that are 'active' or
    // 'today' — this is where the actual reminder/nudge shows up, per the
    // request for a dashboard block that appears "whenever the time comes."
    renderTimelineRemindersCard() {
        const card = document.getElementById('dash-timeline-card');
        if (!card) return;

        const events = DataEngine.getTimelineEvents();
        const active = events
            .map(ev => ({ event: ev, status: this.getTimelineStatus(ev) }))
            .filter(({ status }) => status.state === 'active' || status.state === 'today')
            .sort((a, b) => a.status.daysUntil - b.status.daysUntil);

        if (active.length === 0) {
            card.innerHTML = `
                <p class="reflect-card-label">Timeline Reminders</p>
                <p class="reflect-card-detail">Nothing coming up needs prep yet.</p>
            `;
            return;
        }
        ...
        card.innerHTML = `
            <p class="reflect-card-label">Timeline Reminders</p>
            <ul class="timeline-reminder-list">
                ${active.map(({ event, status }) => `
                    <li class="timeline-reminder-item">
                        <div class="timeline-reminder-title">${event.title} <span class="timeline-reminder-days">${status.state === 'today' ? 'Today' : `${status.daysUntil}d`}</span></div>
                        ${event.note ? `<div class="timeline-reminder-note">${event.note}</div>` : ''}
                    </li>
                `).join('')}
            </ul>
        `;

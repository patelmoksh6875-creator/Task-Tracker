...
export const UIManager = {
    ...
    setupCanvasWorkspace() {
        ...
        // morph the dock down into a thin accent line. Hovering it (handled
        // purely in CSS via .dock-idle:hover) expands it back to full view;
        // moving the mouse away lets it settle back into the thin line.
        // Any interaction on the canvas resets the 5s idle countdown.
        let idleTimer = null;
        const armIdleTimer = () => {
        ...
        document.getElementById('feature-align-grid').addEventListener('click', () => {
            ...
            const goals = DataEngine.getGoals();
            goals.forEach(g => { g.x = snap(g.x); g.y = snap(g.y); });
            DataEngine.saveGoals(goals);

            const tasks = DataEngine.getTasks();
            tasks.forEach(t => { t.x = snap(t.x); t.y = snap(t.y); });
            DataEngine.saveTasks(tasks);
    ...
    wireDragMechanics(element, callback) {
        ...
        const DRAG_THRESHOLD = 4; // px of movement before a mousedown counts as a real drag

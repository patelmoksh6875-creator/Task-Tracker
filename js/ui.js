...
export const UIManager = {
    ...
    setupDashboardPage() {
        ...
        this.renderRogueTaskList(tasks);
        ...
        goals.forEach(goal => {
            ...
            column.innerHTML = `
                ...
                <div class="dashboard-task-list">
                    ${goalTasks.length ? goalTasks.map(t => this.dashboardTaskRow(t)).join('') : '<p class="reflect-card-detail">No open tasks.</p>'}
                </div>
            `;
            container.appendChild(column);
        });
        this.wireDashboardTaskActions(container);
    },

    // Shared row markup for any task shown on the Dashboard — used both
    // inside goal cards and in the standalone Rogue Tasks / carry-over lists.
    dashboardTaskRow(task) {
        return `
            <div class="dashboard-task-row" data-id="${task.id}">
                <span class="dashboard-task-title">${task.title}</span>
                <div class="dashboard-task-actions">
                    <button type="button" class="dashboard-task-edit" title="Edit task" aria-label="Edit task">✎</button>
                    <button type="button" class="dashboard-task-delete" title="Delete task" aria-label="Delete task">×</button>
                </div>
            </div>
        `;
    },

    // Delegated click handling for edit/delete — works for any container
    // that renders .dashboard-task-row elements.
    wireDashboardTaskActions(container) {
        container.addEventListener('click', (e) => {
            const row = e.target.closest('.dashboard-task-row');
            if (!row) return;
            if (e.target.closest('.dashboard-task-edit')) {
                this.editTask(row.dataset.id);
            } else if (e.target.closest('.dashboard-task-delete')) {
                this.deleteTask(row.dataset.id);
            }
        });
    },
    ...
    editTask(taskId) {
        ...
        const newTime = window.prompt('Edit time (optional):', task.timeRange || '');

        task.title = trimmedTitle;
        if (newTime !== null) task.timeRange = newTime.trim() || 'All-Day';
    ...
    deleteTask(taskId) {
        ...
        DataEngine.saveTasks(tasks.filter(t => t.id !== taskId));
        this.setupDashboardPage();
    },
    ...
    renderStreakCard() {
        const card = document.getElementById('dashboard-streak-card');
        if (!card) return;
    ...
    renderFocusWeekCard(log, goals) {
        const card = document.getElementById('dashboard-focus-week-card');
        if (!card) return;
    ...
    renderRogueTaskList(tasks) {
        const list = document.getElementById('dashboard-rogue-list');
        if (!list) return;

        const rogueTasks = tasks.filter(t => t.goalId === null);
        list.innerHTML = rogueTasks.length
            ? rogueTasks.map(t => this.dashboardTaskRow(t)).join('')
            : `<p class="reflect-card-detail">No rogue tasks — everything's tied to a goal.</p>`;

        this.wireDashboardTaskActions(list);
    },

    renderCarryOverList(tasks, goals) {
        const list = document.getElementById('dashboard-carryover-list');
        if (!list) return;
        ...
        if (carried.length === 0) {
            list.innerHTML = `<p class="reflect-card-detail">Nothing carried over — everything's current.</p>`;
            return;
        }

        list.innerHTML = carried.map(({ task, days }) => `
            <div class="dashboard-task-row" data-id="${task.id}">
                <span class="dashboard-task-title">${task.title} <span style="color:var(--text-secondary);">— ${task.goalId ? (goalTitleById[task.goalId] || 'Goal') : 'Rogue'} · ${days}d</span></span>
                <div class="dashboard-task-actions">
                    <button type="button" class="dashboard-task-edit" title="Edit task" aria-label="Edit task">✎</button>
                    <button type="button" class="dashboard-task-delete" title="Delete task" aria-label="Delete task">×</button>
                </div>
    ...
    renderCanvasElements() {
        ...
        goals.forEach(goal => {
            ...
            this.wireResizeHandle(block, block.querySelector('.resize-handle'), 260, 150, (dims) => {
                const currentGoals = DataEngine.getGoals();
                const match = currentGoals.find(g => g.id === goal.id);
                if (match) {
            ...
            block.querySelectorAll('.canvas-task-item').forEach(item => {
                ...
                item.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (e.target === checkBtn || e.target === recurringBtn || e.target === timerBtn) return;
        ...
        tasks.filter(t => t.goalId === null).forEach(task => {
            ...
            this.wireResizeHandle(rogueBlock, rogueBlock.querySelector('.resize-handle'), 210, 100, (dims) => {
                const currentTasks = DataEngine.getTasks();
            ...
            rogueItem.addEventListener('click', (e) => {
                e.stopPropagation();
                if (e.target === rogueCheckBtn || e.target === rogueRecurringBtn || e.target === rogueTimerBtn) return;
    ...
    // Same free-form pan model as dragging a block on the Canvas page, just
    // applied to the whole content layer at once: click-drag on empty
    // background (not a card) translates it in any direction, with no
    // bounds — the canvas is infinite on all four sides.
    wireInfinitePan() {
        const root = document.getElementById('timeline-canvas-root');
        const layer = document.getElementById('timeline-pan-layer');
        if (!root || !layer || root.dataset.panWired) return;
        root.dataset.panWired = 'true';

        let panX = 0, panY = 0;
        let dragging = false, startX = 0, startY = 0, originX = 0, originY = 0;

        root.addEventListener('mousedown', (e) => {
            // Only pan when the click lands on genuinely empty background —
            // not on a card, button, or anything interactive inside a card.
            const bgTargets = [root, layer, document.querySelector('.timeline-viewport'), document.querySelector('.view-header')];
            if (!bgTargets.includes(e.target)) return;
            dragging = true;
